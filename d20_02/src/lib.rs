...
fn solve_problem(input_data: &str) -> i64 {
    ...
    let mut work_data = lines.enumerate().map(|(k, s)|
        NumNode { pos: k, val: s.parse::<i64>().unwrap() * DECRYPTION_KEY }).collect::<Vec<NumNode>>();
        ...
        for i in 0..work_data.len() {
            ...
            if v != 0 {
                if v >= 0 {
                    ...
                    for _ in 0..v % (work_data.len() - 1) as i64 {
                    ...
                    if cfg!(test) {
                        println!("v: {}, {}:{}", v, position, position as i64 + v % work_data.len() as i64);
                    }
                    ...
                    for _ in 0..v.abs() % (work_data.len() - 1) as i64 {
