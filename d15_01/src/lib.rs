...
fn solve_problem(y_level:i32, input_data: &str) -> i32 {
    lazy_static! {
        static ref RE_SENSORS: Regex = Regex::new(r"^Sensor at x=([--\d]+), y=([--\d]+): closest beacon is at x=([--\d]+), y=([--\d]+)$").unwrap();
        //static ref RE_SENSORS: Regex = Regex::new(r"^\s+Starting items: ((?:\d+, )*\d+)$").unwrap();
    }
    ...
    dbg!(zones);
    let mut result = pos_x_right - pos_x_left;
    result
}
